...
//  /Users/jross/Downloads/Slicer4/Slicer4-SuperBuild-Release/SlicerExecutionModel-build/GenerateCLP/bin/GenerateCLP --InputXML /Users/jross/Downloads/ChestImagingPlatformPrivate/CommandLineTools/ComputeCrossSectionalArea/ComputeCrossSectionalArea.xml --OutputCxx /Users/jross/Downloads/ChestImagingPlatformPrivate/CommandLineTools/ComputeCrossSectionalArea/ComputeCrossSectionalAreaCLP.h
//
#include <cstdio>
#include <cstdlib>
#include <iostream>
...
#define GENERATE_TCLAP \
    std::string inFileName = "q"; \
    std::string outFileName = "q"; \
    ...
    fullDescription += "This program can be used to compute cross sectional areas \
of structures in the input label map image. The cross-sectional area is computed with \
respect to the axial plane. The algorithm proceeds by tallying all voxels of various types \
in the label map. The tally for each entity is then multiplied by the in-plane (axial) \
spacing value to give the cross sectional areas. Quantities are printed to std out."; \
 ...
 \
    ...
    msg.str("");msg << "e (default: " << inFileName << ")"; \
    TCLAP::ValueArg<std::string > inFileNameArg("i", "inputFile", msg.str(), 0, inFileName, "std::string", commandLine); \
 \
    msg.str("");msg << "Output CSV file (default: " << outFileName << ")"; \
    TCLAP::ValueArg<std::string > outFileNameArg("o", "outputFile", msg.str(), 0, outFileName, "std::string", commandLine); \
 \
