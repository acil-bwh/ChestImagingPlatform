...
void WriteTagsToFile( std::string, std::vector< std::string >, std::vector< TAGS > );
std::vector< std::string > GetDirectoryList( char* );
...
int main( int argc, char* argv[] )
{
  ...
  //
  std::string rootDicomDirectory = "NA";
  ...
  //
  std::string programDescription = "This program reads a CT (DICOM) image, extracts tags of\
interest and their values and then prints them file.";

  std::string rootDicomDirectoryDescription = "Root directory containing the other dicom directories";
  std::string outputFileNameDescription = "Output csv file containing dicom tag information";
  std::string seriesNumberDescription = "Use this flag to specifiy the series number";
  std::string directoryListDescription = "Specifiy as many input directories as desired using this flag.\
...
and overwritten using the root directory";
  ...
  try
    ...
    rootDicomDirectory = rootDicomDirectoryArg.getValue();
    outputFileName     = outputFileNameArg.getValue();
    seriesNumber       = seriesNumberArg.getValue();

    for ( unsigned int i=0; i<directoryListArg.getValue().size(); i++ )
      {
      directoryList.push_back( directoryListArg.getValue()[i] );
      }
    }
