...
int main( int argc, char *argv[] )
{
  ...
  //
  std::string programDesc = "This program computes statistics needed as inputs to the \
LabelAirwayParticlesByGeneration program. It computes these statistics \
over (possibly) multiple, labeled input airway particles datasets. The \
user must specify information needed to construct the minimum spanning \
tree (which encodes topology over the particles). This information should \
...
compute transition probability scale and angle statics";
...
then normalizing.";
  ...
  try
    ...
    for ( unsigned int i=0; i<inParticlesFileNamesArg.getValue().size(); i++ )
      {
	inParticlesFileNames.push_back( inParticlesFileNamesArg.getValue()[i] );
      }
    }
  ...
  for ( unsigned int i=0; i<inParticlesFileNames.size(); i++ )
      ...
      if ( refParticlesFileName.compare( "NA" ) != 0 )
	{
      ...
      //ViewPolyData( minimumSpanningTree );
      ...
      while ( edgeIt->HasNext() )
	{
	  ...
	  double angle = GetAngleBetweenVectors( direction1, direction2, true );
		    ...
		    double* weight, double particleDistanceThreshold, double edgeWeightAngleSigma )
{
  ...
  double connectorMagnitude = GetVectorMagnitude( connectingVec );
  ...
  double angle1 =  GetAngleBetweenVectors( particle1Hevec2, connectingVec, true );
  double angle2 =  GetAngleBetweenVectors( particle2Hevec2, connectingVec, true );
...
void ViewPolyData( vtkSmartPointer< vtkMutableUndirectedGraph > graph )
{
  // vtkIndent indent;
      ...
      for ( unsigned int j=0; j<numParticles; j++ )
	{
	  ...
	  if ( refParticleType == particleType )
		...
		vec[2] = refParticles->GetPoint(i)[2] - particles->GetPoint(j)[2];
	      ...
	      double dist = GetVectorMagnitude( vec );
	      if ( dist <= epsilon )
		{		  
		  ...
		  EMISSION emissionData;
		    ...
		    emissionData.angle = GetAngleBetweenVectors( direction1, direction2, true );
...
void WriteEmissionProbabilityData( std::map< unsigned char, std::vector< EMISSION > > emissionProbData, std::string fileName )
{
  ...
  while ( mapIt != emissionProbData.end() )
      ...
      if ( numEntries == 0 )
	{
      ...
      for ( unsigned int i=0; i<numEntries; i++ )
	{
	  // These if-statements are a check against NaN values. Should be dealt
