  ...
  if ( printDiceScores )
    {
      ...
      for ( unsigned int p=0; p<particlesReader->GetOutput()->GetNumberOfPoints(); p++ )
      	{
  	  inType  = (unsigned char)( particlesReader->GetOutput()->GetPointData()->GetArray( "ChestType" )->GetTuple(p)[0] );
  	  outType = (unsigned char)( particlesToGenLabeledParticles->GetOutput()->GetPointData()->GetArray( "ChestType" )->GetTuple(p)[0] );
