	...
	vtkSmartPointer< vtkPolyData > inParticles, vtkSmartPointer< vtkPolyData > outParticles)
{
	...
	for (unsigned int i = 0; i<numberPointDataArrays; i++)
	{
		...
		{			
			ChestRegionChestTypeArray->SetNumberOfComponents(inParticles->GetPointData()->GetArray(i)->GetNumberOfComponents());
			ChestRegionChestTypeArray->SetName(inParticles->GetPointData()->GetArray(i)->GetName());
		}
		else
		{
		...
		if (labelValue > 0)
		{
			for (unsigned int k = 0; k<cipRegions.size(); k++)
			{
					...
					conventions.CheckSubordinateSuperiorChestRegionRelationship(labelRegion, cipRegion))
				{
					...
					{
						std::string name(inParticles->GetPointData()->GetArray(j)->GetName());
						if (name.compare("ChestRegionChestType") == 0)
						{
							...
							unsigned short newChestRegionChestTypeValue =
								(unsigned short)(conventions.GetValueFromChestRegionAndType(cipRegion, cipType));

							ChestRegionChestTypeArray->InsertTypedTuple(inc, &newChestRegionChestTypeValue);
						}
						else
						{
	...
	for (unsigned int j = 0; j<numberPointDataArrays-1; j++)
	{
		outParticles->GetPointData()->AddArray(arrayVec[j]);
	}
	outParticles->GetPointData()->AddArray(ChestRegionChestTypeArray);
}
	...
	vtkSmartPointer< vtkPolyData > inParticles, vtkSmartPointer< vtkPolyData > outParticles)
{
	...
	vtkSmartPointer< vtkUnsignedShortArray > pointChestRegionChestTypeArray = vtkSmartPointer< vtkUnsignedShortArray >::New();
	...
	for (unsigned int i = 0; i<numberPointDataArrays; i++)
	{
		...
		if (name.compare("ChestRegionChestType") == 0)
		{
			pointChestRegionChestTypeArray->SetNumberOfComponents(inParticles->GetPointData()->GetArray(i)->GetNumberOfComponents());
			pointChestRegionChestTypeArray->SetName(inParticles->GetPointData()->GetArray(i)->GetName());
		}
		else
		{
			vtkSmartPointer< vtkFloatArray > array = vtkSmartPointer< vtkFloatArray >::New();
		...
		if (add)
		{
			...
			for (unsigned int j = 0; j<numberPointDataArrays; j++)
			{
				...
				if (name.compare("ChestRegionChestType") == 0)
				{
					...
					pointChestRegionChestTypeArray->InsertTypedTuple(inc, &newChestRegionChestTypeValue);
				}
				else
				{
	...
	outParticles->GetPointData()->AddArray(pointChestRegionChestTypeArray);
}
