...
class ExtractParticlesFromChestRegionChestTypeInputSpec(CommandLineInputSpec):
    labelMap = File(desc="Input label map file name. This is an optional input. If no label map is specified,\n      the 'ChestRegion' and 'ChestType' arrays in the input will be used to extract the region or type specified\n      with the '-r' and '-t' flags, respectively ", exists=True, argstr="--labelMap %s")
    inParticles = traits.Str(desc="Input particles file name", argstr="--inParticles %s")
    outParticles = traits.Str(desc="Output particles file name", argstr="--outParticles %s")
    region = InputMultiPath(traits.Str, desc="Chest regions from which to extract particles", sep=",", argstr="--region %s")
    type = InputMultiPath(traits.Str, desc="Chest types for which to extract particles. If specifying a label map this flag \n      is not relevent.", sep=",", argstr="--type %s")
...
class PerformMorphologicalInputSpec(CommandLineInputSpec):
    opt_in = File(desc="Input label map file name.", exists=True, argstr="--in %s")
    out = traits.Either(traits.Bool, File(), hash_files=False, desc="Output label map file name.", argstr="--out %s")
    cl = traits.Bool(desc="Set to 1 to perform morphological closing. Default: False. Only one morphological operation can be specified. Operations are considered in the following order: dilate, erode, open, close. E.g., if dilation is specified, only it will be performed regardless of whether the other operations have requested.", argstr="--cl ")
    dilate = traits.Bool(desc="Set to 1 to perform morphological dilation. Default: False. Only one morphological operation can be specified. Operations are considered in the following order: dilate, erode, open, close. E.g., if dilation is specified, only it will be performed regardless of whether the other operations have requested.", argstr="--dilate ")
    erode = traits.Bool(desc="Set to 1 to perform morphological erosion. Default: False. Only one morphological operation can be specified. Operations are considered in the following order: dilate, erode, open, close. E.g., if dilation is specified, only it will be performed regardless of whether the other operations have requested.", argstr="--erode ")
    op = traits.Bool(desc="Set to 1 to perform morphological opening. Default: False. Only one morphological operation can be specified. Operations are considered in the following order: dilate, erode, open, close. E.g., if dilation is specified, only it will be performed regardless of whether the other operations have requested.", argstr="--op ")
    ...
    region = InputMultiPath(traits.Int, desc="Users must specify chest-region chest-type pairs. only labels corresponding to the pairs will be operated on. Use this flag to specify the chest region of a given pair. Each time this flag is used, the -t or --type flag should also be used to specify the corresponding type", sep=",", argstr="--region %s")
    type = InputMultiPath(traits.Int, desc="Users must specify chest-region chest-type pairs; only labels corresponding to the pairs will be operated on. Use this flag to specify the chest type of a given pair. Each time this flag is used, the -r or --region flag should also be used to specify the corresponding region", sep=",", argstr="--type %s")
    all = traits.Bool(desc="If set to 1, then all chest-region chest-type pairs present in the input label map will be processed. Any region-type pairs specified using the -r (--region) and -t (--type) flags will be ignored. (0 by default).", argstr="--all ")
...
class GetStructuresInLabelMapInputSpec(CommandLineInputSpec):
    input = traits.Str(desc="Input label map file name.", argstr="--input %s")
...
class QualityControlInputSpec(CommandLineInputSpec):
    ct = File(desc="Input CT image file name. Only needs to be specified if you intend to create lung lobe QC images", exists=True, argstr="--ct %s")
    labelMap = File(desc="Input label map file name", exists=True, argstr="--labelMap %s")
    lungProj = traits.Either(traits.Bool, File(), hash_files=False, desc="Lung projection image output image file name", argstr="--lungProj %s")
    airwayProj = traits.Either(traits.Bool, File(), hash_files=False, desc="Airway projection output image file name", argstr="--airwayProj %s")
...
class QualityControlOutputSpec(TraitedSpec):
    lungProj = File(desc="Lung projection image output image file name", exists=True)
    airwayProj = File(desc="Airway projection output image file name", exists=True)
...
"""
    ...
    _cmd = " QualityControl "
    _outputs_filenames = {'lungProj':'lungProj.nii','airwayProj':'airwayProj.nii'}
...
class FilterFissureParticleDataInputSpec(CommandLineInputSpec):
    ...
    spacing = traits.Float(desc="This value indicates the inter-particle spacing of the input data set", argstr="--spacing %f")
...
class GenerateModelInputSpec(CommandLineInputSpec):
    opt_in = traits.Str(desc="Input mask file name", argstr="--in %s")
    out = traits.Str(desc="Output model file name", argstr="--out %s")
    ...
    origSp = traits.Bool(desc="Set to 1 to used standard origin and spacing. Set to 0 by default.", argstr="--origSp ")
...
class FilterAirwayParticleDataInputSpec(CommandLineInputSpec):
    ...
    spacing = traits.Float(desc="This value indicates the inter-particle spacing of the input data set ", argstr="--spacing %f")
    distance = traits.Float(desc="Maximum inter-particle distance. Two particles must be at least this close \n                together to be considered for connectivity. ", argstr="--distance %f")
    ...
    scaleRatio = traits.Float(desc="Scale ratio threshold in the interval [0,1]. This value indicates the degree to which \n                two particles can differ in scale and still be considered for connectivity. The higher the value, the more permisse the filter is \n                with respect to scale differences.", argstr="--scaleRatio %f")
...
class GenerateLobeSurfaceModelsInputSpec(CommandLineInputSpec):
    inFileName = File(desc="Input label map to serve as starting point for lung lobe \n      segmentation .The level of lung segmentation 'grunularity' should be at the left lung right lung split \n      level. In other words, a mask divided into thirds (left and right) will work fine, as will one for which \n      only the left and right lung are labeled", exists=True, argstr="--inFileName %s")
    refim = File(desc="Reference image label map corresponding to the reference points. \n      This is the image to which all other image data in the training set is registered to.", exists=True, argstr="--refim %s")
    resourcesDir = traits.Str(desc="Directory containing resources needed to build the shape model. \n      This directory is expected to contain a collection of file pairs: a transform file and a \n      region and type points file. Each pair corresponds to a single case. The transform file \n      indicates how to transform the case so that it aligns with a reference case and is expected \n      to have the extension '_TO_10002K_INSP_STD_BWH_COPD_transform.txt'. The region and type \n      points file contains physical coordinates along the right horizontal, right oblique, and left oblique \n      lobe boundaries. It is expected to have the extension '_regionAndTypePoints.csv'. For a given pair \n      the prefix is expected to be the same.", argstr="--resourcesDir %s")
    opt_or = traits.Str(desc="Output right oblique and right horizontal shape model file name. The first \n      half of the entries in the saved fill will correspond to the right oblique boundary surface. \n      The second half of the entries will correspond to the right horizontal boundary surface.", argstr="--or %s")
    ol = traits.Str(desc="Output left lung shape model file name. This file will contain the shape model \n      describing the boundary between the left upper and left lower lobes.", argstr="--ol %s")
...
class ResampleLabelMap3DInputSpec(CommandLineInputSpec):
...
class ResampleLabelMap3DOutputSpec(TraitedSpec):
    pass


class ResampleLabelMap3D(SEMLikeCommandLine):
...
"""
    ...
    input_spec = ResampleLabelMap3DInputSpec
    output_spec = ResampleLabelMap3DOutputSpec
    _cmd = " ResampleLabelMap3D "
    _outputs_filenames = {}
...
class PerturbParticlesInputSpec(CommandLineInputSpec):
    input = File(desc="Input particles file name.", exists=True, argstr="--input %s")
    output = traits.Either(traits.Bool, File(), hash_files=False, desc="Output particles file name.", argstr="--output %s")
...
class PerturbParticlesOutputSpec(TraitedSpec):
    output = File(desc="Output particles file name.", exists=True)
...
description: This program can used to perturb a particles dataset. This kind of operation can be useful for experimentation purposes. Currently, the program simply translates the particles dataset by a random offset in the x, y, and z directions. The user can control the magnitude of the random offset that is used.
...
class EnhanceFissuresInImageInputSpec(CommandLineInputSpec):
    lm = File(desc="Input lung label map file name. On CT voxels in the foreground region of this \n      label map will be enhanced.", exists=True, argstr="--lm %s")
    ct = File(desc="Input CT file to be enhanced.", exists=True, argstr="--ct %s")
    vp = traits.Str(desc="Vessel particles to use for the enhancement.", argstr="--vp %s")
...
class TransferFieldDataToFromPointDataInputSpec(CommandLineInputSpec):
    ...
    mf = traits.Bool(desc="Setting this to true will maintain the field data. Setting it to false will eliminate the field data from the output. Only relevant if requesting to transfer field data to point data.", argstr="--mf ")
    mp = traits.Bool(desc="Setting this to true will maintain the point data. Setting it to false will eliminate the point data from the output. Only relevant if requesting to transfer point data to field data.", argstr="--mp ")
    fp = traits.Bool(desc="Set to true to transfer field data to point data.", argstr="--fp ")
    pf = traits.Bool(desc="Set to true to transfer point data to field data.", argstr="--pf ")
...
class ComputeCrossSectionalAreaInputSpec(CommandLineInputSpec):
    inputFile = traits.Str(desc="e", argstr="--inputFile %s")
...
class FitLobeSurfaceModelsToParticleDataInputSpec(CommandLineInputSpec):
    lf = traits.Str(desc="Left lung fissure particles file name", argstr="--lf %s")
    lv = traits.Str(desc="Left lung vessel particles file name", argstr="--lv %s")
    la = traits.Str(desc="Left lung airway particles file name", argstr="--la %s")
    rf = traits.Str(desc="Right lung fissure particles file name", argstr="--rf %s")
    rv = traits.Str(desc="Right lung vessel particles file name", argstr="--rv %s")
    ra = traits.Str(desc="Right lung airway particles file name", argstr="--ra %s")
    ilm = traits.Str(desc="Input left lobes surface model file name", argstr="--ilm %s")
    irm = traits.Str(desc="Input right lobes surface model file name", argstr="--irm %s")
    olm = traits.Str(desc="Output left lobes surface model file name", argstr="--olm %s")
    orm = traits.Str(desc="Output right lobes surface model file name", argstr="--orm %s")
    shapeVar = traits.Float(desc="Shape variance threshold. This indicates how much of \n      the variance you want accounted for during the shape model fitting process", argstr="--shapeVar %f")
    ...
    numIters = traits.Int(desc="Number of iterations to perform for Nelder-Mead simplex model fitting", argstr="--numIters %d")
...
class CropLungInputSpec(CommandLineInputSpec):
    inFileName = File(desc="Input CT file", exists=True, argstr="--inFileName %s")
    plf = File(desc="Label map file name", exists=True, argstr="--plf %s")
    outFileName = traits.Either(traits.Bool, File(), hash_files=False, desc="Output Cropped CT volume", argstr="--outFileName %s")
    opl = traits.Either(traits.Bool, File(), hash_files=False, desc="Ouput label map volume", argstr="--opl %s")
    region = InputMultiPath(traits.Str, desc="Specify a region you want to crop (ex: WholeLung)", sep=",", argstr="--region %s")
    type = InputMultiPath(traits.Str, desc="Specify a type you want to crop (ex:Airway)", sep=",", argstr="--type %s")
    regionPair = InputMultiPath(traits.Str, desc="Specify a region in a region type pair you want to crop. This flag should be used together with the typePair flag", sep=",", argstr="--regionPair %s")
    typePair = InputMultiPath(traits.Str, desc="Specify a type in a region type pair you want to crop. This flag should be used together with the regionPair flag", sep=",", argstr="--typePair %s")
...
class CropLungOutputSpec(TraitedSpec):
    outFileName = File(desc="Output Cropped CT volume", exists=True)
    opl = File(desc="Ouput label map volume", exists=True)
...
"""
    ...
    _cmd = " CropLung "
    _outputs_filenames = {'outFileName':'outFileName.nii','opl':'opl.nii'}


class ComputeIntensityStatisticsInputSpec(CommandLineInputSpec):
    labelMapFileName = traits.Str(desc="Input label map file name", argstr="--labelMapFileName %s")
    ctFileName = traits.Str(desc="Input CT file name", argstr="--ctFileName %s")
...
class ClassifyFissureParticlesInputSpec(CommandLineInputSpec):
    ...
    lm = traits.Str(desc="Left lung shape model file name", argstr="--lm %s")
    rm = traits.Str(desc="Right lung shape model file name", argstr="--rm %s")
    loClassified = traits.Either(traits.Bool, File(), hash_files=False, desc="Left oblique classified particles file name", argstr="--loClassified %s")
    roClassified = traits.Either(traits.Bool, File(), hash_files=False, desc="Right oblique classified particles file name", argstr="--roClassified %s")
    rhClassified = traits.Either(traits.Bool, File(), hash_files=False, desc="Right horizontal classified particles file name", argstr="--rhClassified %s")
...
class ClassifyFissureParticlesOutputSpec(TraitedSpec):
    loClassified = File(desc="Left oblique classified particles file name", exists=True)
    roClassified = File(desc="Right oblique classified particles file name", exists=True)
    rhClassified = File(desc="Right horizontal classified particles file name", exists=True)
...
"""
    ...
    _outputs_filenames = {'roClassified':'roClassified.vtk','loClassified':'loClassified.vtk','rhClassified':'rhClassified.vtk'}
...
class LabelParticlesByChestRegionChestTypeInputSpec(CommandLineInputSpec):
    inParticles = File(desc="Input particles file name.", exists=True, argstr="--inParticles %s")
    outParticles = File(desc="Output particles file name.", exists=True, argstr="--outParticles %s")
    labelMap = File(desc="Input label map file name. If specified the 'ChestRegion' value will be determined from the label map", exists=True, argstr="--labelMap %s")
    region = traits.Int(desc="Chest region for particles labeling. UNDEFINEDREGION by default", argstr="--region %d")
    type = traits.Int(desc="Chest type for particles labeling. UNDEFINEDTYPE by default", argstr="--type %d")
...
description: This program is used to label particles datasets by chest region and chest type. The user must specify the type of the input particles, but the chest region can either be determined by an input label map or be specified at the command line.
...
class GenerateOtsuLungCastInputSpec(CommandLineInputSpec):
    ct = File(desc="Input CT image file name", exists=True, argstr="--ct %s")
    dir = traits.Str(desc="Directory containing DICOM images to segment.", argstr="--dir %s")
    mask = traits.Either(traits.Bool, File(), hash_files=False, desc="Output lung mask file name", argstr="--mask %s")
...
class GenerateOtsuLungCastOutputSpec(TraitedSpec):
    mask = File(desc="Output lung mask file name", exists=True)
...
"""
    ...
    _outputs_filenames = {'mask':'mask.nii'}


class RegisterLabelMaps3DInputSpec(CommandLineInputSpec):
    fixedImageFileName = File(desc="fixed Image File Name", exists=True, argstr="--fixedImageFileName %s")
    movingImageFileName = File(desc="moving Image FileName", exists=True, argstr="--movingImageFileName %s")
    outputImage = traits.Either(traits.Bool, File(), hash_files=False, desc="Output image file name", argstr="--outputImage %s")
    outputTransform = traits.Either(traits.Bool, File(), hash_files=False, desc="Output transform file name", argstr="--outputTransform %s")
    ...
    fixedImageID = traits.Str(desc="Fixed Image subject ID. If not specified, the subject ID will be automatically extracted from the file path", argstr="--fixedImageID %s")
    registrationID = traits.Str(desc="unique Id for the registration process.", argstr="--registrationID %s")
    downsampleFactor = traits.Float(desc="downsampleFactor", argstr="--downsampleFactor %f")
    max = traits.Float(desc="maxStepLength", argstr="--max %f")
    min = traits.Float(desc="minStepLength", argstr="--min %f")
    numberOfIterations = traits.Int(desc="Number of iterations. ", argstr="--numberOfIterations %d")
    translationScale = traits.Float(desc="translationScale", argstr="--translationScale %f")
...
class RegisterLabelMaps3DOutputSpec(TraitedSpec):
    outputImage = File(desc="Output image file name", exists=True)
    outputTransform = File(desc="Output transform file name", exists=True)


class RegisterLabelMaps3D(SEMLikeCommandLine):
...
"""
    ...
    input_spec = RegisterLabelMaps3DInputSpec
    output_spec = RegisterLabelMaps3DOutputSpec
    _cmd = " RegisterLabelMaps3D "
    _outputs_filenames = {'outputImage':'outputImage.nii','outputTransform':'outputTransform.mat'}


class ResampleLabelMap2DInputSpec(CommandLineInputSpec):
    labelMapFile = File(desc="Label map file name to resample", exists=True, argstr="--labelMapFile %s")
    inputTransform = InputMultiPath(traits.Str, desc="Input transform file name(s). The last transform entered will be applied first", sep=",", argstr="--inputTransform %s")
    resampledFile = File(desc="Resampled label map (output) file name", exists=True, argstr="--resampledFile %s")
    destinationFile = File(desc="Label map file name to resampDestinatin file name. This should be a header file that contains the necessary information (image spacing, origin, and size) for the resampling process.", exists=True, argstr="--destinationFile %s")
...
class ResampleLabelMap2DOutputSpec(TraitedSpec):
    pass


class ResampleLabelMap2D(SEMLikeCommandLine):
    """title: ResampleLabelMap
...
"""
    ...
    input_spec = ResampleLabelMap2DInputSpec
    output_spec = ResampleLabelMap2DOutputSpec
    _cmd = " ResampleLabelMap2D "
    _outputs_filenames = {}
...
class GenerateSimpleLungMaskInputSpec(CommandLineInputSpec):
    inputVolume = File(position=-3, desc="Input Chest CT volume", exists=True, argstr="%s")
    outputVolume = traits.Either(traits.Bool, File(), position=-1, hash_files=False, desc="Output Lung Mask", argstr="%s")


class GenerateSimpleLungMaskOutputSpec(TraitedSpec):
    outputVolume = File(position=-1, desc="Output Lung Mask", exists=True)
...
class GenerateStenciledLabelMapFromParticlesInputSpec(CommandLineInputSpec):
    inLabelMap = File(desc="Input label map file name. Used to retrieve spacing, origin, and dimensions for creating output label map", exists=True, argstr="--inLabelMap %s")
    outLabelMap = traits.Either(traits.Bool, File(), hash_files=False, desc="Output label map file name", argstr="--outLabelMap %s")
    particles = File(desc="Input particles file name", exists=True, argstr="--particles %s")
    ctSigma = traits.Float(desc="The CT scanner point spread function sigma. 0.0 by default.", argstr="--ctSigma %f")
    radius = traits.Float(desc="Stencil radius in mm.", argstr="--radius %f")
    height = traits.Float(desc="Cylinder stencil height in mm. Default is 1mm. This should typically be set to the inter-particle distance.", argstr="--height %f")
    vessel = traits.Bool(desc="Set this flag to indicate that in the input particles correspond to vessels. Default: False.", argstr="--vessel ")
    airway = traits.Bool(desc="Set this flag to indicate that in the input particles correspond to airways. Default: False.", argstr="--airway ")
    fissure = traits.Bool(desc="Set this flag to indicate that in the input particles correspond to fissures. Default: False.", argstr="--fissure ")
    sphere = traits.Bool(desc="Set this flag to indicate that the sphere stencil should be used. Default: False.", argstr="--sphere ")
    cylinder = traits.Bool(desc="Set this flag to indicate that the cylinder stencil should be used. Default: False.", argstr="--cylinder ")
    scale = traits.Bool(desc="Setting this flag will cause the stencil pattern to be scaled according to particle scale. If set, any radius value specified using the -r flag will be ignored. Scaling will be performed using predetermined equations relating particle scale and CT point spread function sigma (set using the -ctSigma flag). Default: False.", argstr="--scale ")


class GenerateStenciledLabelMapFromParticlesOutputSpec(TraitedSpec):
    outLabelMap = File(desc="Output label map file name", exists=True)
...
class ComputeFissureFeatureVectorsInputSpec(CommandLineInputSpec):
    ...
    rm = traits.Str(desc="Right lung lobe boundaries shape model file name. The mean points of the shape model will \n      be used to create thin plate spline surfaces. These surfaces will be used to compute certain features. A right \n      shape model file OR a left shape model file (but not both) must be specified.", argstr="--rm %s")
    lm = traits.Str(desc="Left lung lobe boundaries shape model file name. The mean points of the shape model will \n      be used to create a thin plate spline surface. This surface will be used to compute certain features. A right \n      shape model file OR a left shape model file (but not both) must be specified.", argstr="--lm %s")
    out = traits.Str(desc="Comma-separated value (csv) file that will contain the feature vectors \n      for each point location indicated by the input polydata file.", argstr="--out %s")
...
class RegisterLabelMaps2DInputSpec(CommandLineInputSpec):
    fixedImageFileName = File(desc="fixed Image File Name", exists=True, argstr="--fixedImageFileName %s")
    movingImageFileName = File(desc="moving Image FileName", exists=True, argstr="--movingImageFileName %s")
    outputTransform = traits.Either(traits.Bool, File(), hash_files=False, desc="Output transform file name", argstr="--outputTransform %s")
    outputImage = traits.Either(traits.Bool, File(), hash_files=False, desc="Output image file name", argstr="--outputImage %s")
    fixedLabelmapFileName = File(desc="fixed LabelMap FileName", exists=True, argstr="--fixedLabelmapFileName %s")
    movingImageID = traits.Str(desc="Moving Image subject ID. If not specified, the subject ID will be automatically extracted from the file path", argstr="--movingImageID %s")
    fixedImageID = traits.Str(desc="Fixed Image subject ID. If not specified, the subject ID will be automatically extracted from the file path", argstr="--fixedImageID %s")
    registrationID = traits.Str(desc="unique Id for the registration process.", argstr="--registrationID %s")
    degrees = traits.Float(desc="downsampleFactor", argstr="--degrees %f")
...
class RegisterLabelMaps2DOutputSpec(TraitedSpec):
    outputTransform = File(desc="Output transform file name", exists=True)
    outputImage = File(desc="Output image file name", exists=True)


class RegisterLabelMaps2D(SEMLikeCommandLine):
    """title: RegisterLabelMaps2D
...
"""
    ...
    input_spec = RegisterLabelMaps2DInputSpec
    output_spec = RegisterLabelMaps2DOutputSpec
    _cmd = " RegisterLabelMaps2D "
    _outputs_filenames = {'outputImage':'outputImage.nii','outputTransform':'outputTransform.mat'}
...
class SegmentLungLobesInputSpec(CommandLineInputSpec):
    opt_in = File(desc="Input lung label map file name. Left and right lungs must be uniquely labeled.", exists=True, argstr="--in %s")
    out = File(desc="Output label map file name with the lung lobe segmented.", exists=True, argstr="--out %s")
    lm = traits.Str(desc="File name of shape model describing the boundary between the lobes \n      in the left lung.", argstr="--lm %s")
    rm = traits.Str(desc="File name of shape model describing the boundaries between the lobes \n      in the right lung.", argstr="--rm %s")
    rightHorizontalFiducials = InputMultiPath(traits.List(traits.Float(), minlen=3, maxlen=3), desc="Fiducals along right horizontal fissure", argstr="--rightHorizontalFiducials %s...")
    rightObliqueFiducials = InputMultiPath(traits.List(traits.Float(), minlen=3, maxlen=3), desc="Fiducials along right oblique fissure", argstr="--rightObliqueFiducials %s...")
    leftObliqueFiducials = InputMultiPath(traits.List(traits.Float(), minlen=3, maxlen=3), desc="Fiducials along left oblique fissure", argstr="--leftObliqueFiducials %s...")
    loParticles = traits.Str(desc="Left oblique particles file name. It's assumed that all particles are \'highly likely\' to \n      represent the left oblique fissure (i.e. essentially no noise particles). An interpolating surface will be fit \n      through these points to define the boundary between the lobes.", argstr="--loParticles %s")
    roParticles = traits.Str(desc="Right oblique particles file name. It's assumed that all particles are \'highly likely\' to \n      represent the right oblique fissure (i.e. essentially no noise particles). An interpolating surface will be fit \n      through these points to define the boundary between the lobes.", argstr="--roParticles %s")
    rhParticles = traits.Str(desc="Right horizontal particles file name. It's assumed that all particles are \'highly likely\' to \n      represent the right horizontal fissure (i.e. essentially no noise particles). An interpolating surface will be fit \n      through these points to define the boundary between the lobes.", argstr="--rhParticles %s")
    regionType = traits.Str(desc="Region and type points file indicating a set of points that lying on the lobe boundaries. \n      If specified, these points will be used in addition to any particle points supplied in order to form a full set of \n      points along the lobe boundaries through which interpolating surfaces will be fit.", argstr="--regionType %s")
...
class GenerateMedianFilteredImageInputSpec(CommandLineInputSpec):
    ...
    lowerClipValue = InputMultiPath(traits.Int, desc="Lower clip value applied to input image before filtering. This flag should be followed by two values: the first value is the clip value and the second value is the replacement value (i.e., everything below the clip value will be assigned the replacement value)", sep=",", argstr="--lowerClipValue %s")
    upperClipValue = InputMultiPath(traits.Int, desc="Upper clip value applied to input image before filtering. This flag should be followed by two values: the first value is the clip value and the second value is the replacement value (i.e., everything below the clip value will be assigned the replacement value)", sep=",", argstr="--upperClipValue %s")
    inputFile = traits.Str(desc="Input CT file name", argstr="--inputFile %s")
    outputFile = traits.Str(desc="Output CT file name", argstr="--outputFile %s")
...
class ScourParticleDataInputSpec(CommandLineInputSpec):
    opt_in = File(desc="Input particles file name.", exists=True, argstr="--in %s")
    out = File(desc="Output particles file name.", exists=True, argstr="--out %s")
    name = InputMultiPath(traits.Str, desc="Specify the name(s) of the arrays to be removed", sep=",", argstr="--name %s")


class ScourParticleDataOutputSpec(TraitedSpec):
    pass


class ScourParticleData(SEMLikeCommandLine):
    """title: ScourParticleData

category: Chest Imaging Platform.Toolkit.Particles

description: This program can be used to scour an input particles data set, removing specified field data arrays.
...
"""
    ...
    input_spec = ScourParticleDataInputSpec
    output_spec = ScourParticleDataOutputSpec
    _cmd = " ScourParticleData "
    _outputs_filenames = {}
...
class GeneratePartialLungLabelMapInputSpec(CommandLineInputSpec):
    out = traits.Either(traits.Bool, File(), hash_files=False, desc="Output segmentation file name", argstr="--out %s")
    ct = File(desc="CT image file name", exists=True, argstr="--ct %s")
    ...
    lsr = traits.Int(desc="Radius used to split the left and right lungs.", argstr="--lsr %d")
    ...
    radius = traits.Int(desc="Radius used for left-right lung splitting. Larger values lead to more aggressive \n      splitting, but may slightly degrade the quality of the seqmentation in the splitting region", argstr="--radius %d")
    ff = traits.Bool(desc="Indicates if scan is feet first.", argstr="--ff ")


class GeneratePartialLungLabelMapOutputSpec(TraitedSpec):
    out = File(desc="Output segmentation file name", exists=True)
