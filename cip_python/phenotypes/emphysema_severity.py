...
class EmphysemaSeverityIndex:
    """ General purpose class implementing the emphysema severity index. Training data with features consisting on 6 emphysema subtype
    ...
    interp_method: string
        ...
        Choice should be : linear, spline, nearest (so far only spline is defined)
        
    multipath: boolean
        Set to True if multiple paths (4 to be specific) as opposed to 1 severity path is used. 
    ...
    def compute_emphysema_severity_index(self, the_features, the_model):
        ...
        for kk in range(0,np.shape(the_features)[0]):
            [ST[kk,0],ST[kk,1]] = self.computeSTcoordinates(embeddedData[kk,:],x0,poly_a,poly_b,poly_c)
        
        return ST

                                                                                                                                                                                                      
    def predict(self, testing_features):
        """ Compute severity on testing data given models. equivalent to computeemphysemaSeverityIndex
        ...
        if (self.single_path):
            ...
            ST = self.compute_emphysema_severity_index(testing_features, self.final_model)
        else:
            print(" using interpolated model to compute severity")
            ST=self.computeSTCoordinateInterpModel(testing_features,self.final_model)
            
        return ST
        

    def emphysemaSeverityIndexTrajectories(self, training,nClusters,dimSubspace,ctrsinit, model_ref=None):
        ...
        for kk in range(0,np.shape(ldacc)[0]):
            [ST[kk,0],ST[kk,1]] = self.computeSTcoordinates(ldacc[kk,:],x0,poly_a,poly_b,poly_c)
    ...
    def computeSTcoordinates(self, p,x0,a,b,c):
        """ Compute the severity and normal values given polynomial coefficients
    ...
    def evalInterpModel(self, interp_model,t_val):
        ...
        ----------
        interp_model: dict
            an interpolation model, consisting of a set of coefficients to interpolate from
        ...
        if (interp_method == 'spline'):
            modeli['poly'][0] = PchipInterpolator(interp_model['Ti'],interp_model['ai'],extrapolate=True)(t_val)[0]
            modeli['poly'][1] = PchipInterpolator(interp_model['Ti'],interp_model['bi'],extrapolate=True)(t_val)[0]
	    modeli['poly'][2] = PchipInterpolator(interp_model['Ti'],interp_model['ci'],extrapolate=True)(t_val)[0]
    ...
    def optimize_interp_model(self,p,interp_model):                    
        """ perform the potimization in order to obtain the interpolated model at point p in LDA space
        ...
        res  = op.minimize(fun = self.compute_local_N_given_global_t_param, x0 = global_t_param, args = (p,interp_model), method = 'Nelder-Mead')   
        t_param = res.x
    ...
    def compute_local_N_given_global_t_param(self,t_value,p, im):
        ...
        [Local_S,local_N]=self.computeSTcoordinates(p,model_ii['min'],model_ii['poly'][2], model_ii['poly'][1],model_ii['poly'][0])
    ...
    def computeSTCoordinateInterpModel(self,the_features,interp_model):
        ...
        for kk in range(0,np.shape(the_features)[0]):
            ...
            [ST[kk,0],ST[kk,1]] = self.computeSTcoordinates(p,model_ii['min'],model_ii['poly'][2], model_ii['poly'][1],model_ii['poly'][0])     
    ...
    def computeInterpModel(self, models,model0):
        ...
        for kk in range(0,4):
            [STe[kk,0],STe[kk,1]]=self.computeSTcoordinates([xe[kk],ye[kk]],x0,a,b,c);
    ...
    def fit(self, training_features):
        ...
        if (self.single_path):
            ...
            """ build 4 models with the training data model is now an interpolated model""" 
            model[0]=model0
            num_levels = 4
            for level_num in range (1,(num_levels+1)):
                for node_in_level in range (0,np.power(2,level_num)):

                    current_node_number = np.power(2,level_num)+node_in_level-1# because we are starting at node 0 in python
                    current_ST = current_node_number
                    ST_to_split = int(np.floor((np.float(current_ST)+1.0)/2.0))-1
                    st_inclusion = np.ones(np.shape(ST[0])[0])
                    for current_split_level in range(level_num,0,-1): # not counting root node, root node is 0
                        if ((np.float(current_ST+1)/np.float(ST_to_split+1)) ==2): # to the left -> I am an uppser split -> set lower split to 0
