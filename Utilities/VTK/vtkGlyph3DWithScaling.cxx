/*=========================================================================
  ...
  Module:    vtkGlyph3DWithScaling.cxx
...
vtkStandardNewMacro(vtkGlyph3DWithScaling);
vtkCxxSetObjectMacro(vtkGlyph3DWithScaling, SourceTransform, vtkTransform);
...
vtkGlyph3DWithScaling::vtkGlyph3DWithScaling()
{
  this->Scaling = 1;
  this->ScalingX = 1;
  this->ScalingY = 1;
...
//----------------------------------------------------------------------------
int vtkGlyph3DWithScaling::RequestData(
...
//----------------------------------------------------------------------------
bool vtkGlyph3DWithScaling::Execute(
  vtkDataSet* input,
  ...
  for (inPtId=0; inPtId < numPts; inPtId++)
    ...
    if (inSScalars && (this->ColorMode == VTK_COLOR_BY_SCALE))
      ...
      for (i=0; i < numSourcePts; i++)
        ...
        else if (this->ScalingY == 1)
          newScalars->InsertTuple(i+ptIncr, &scaley);
        else if (this->ScalingZ == 1)
          newScalars->InsertTuple(i+ptIncr, &scalez);
        else
...
void vtkGlyph3DWithScaling::SetSourceConnection(int id, vtkAlgorithmOutput* algOutput)
{
  if (id < 0)
    {
...
//----------------------------------------------------------------------------
// Specify a source object at a specified table location.
void vtkGlyph3DWithScaling::SetSourceData(int id, vtkPolyData *pd)
{
  int numConnections = this->GetNumberOfInputConnections(1);
...
vtkPolyData *vtkGlyph3DWithScaling::GetSource(int id)
{
...
//----------------------------------------------------------------------------
void vtkGlyph3DWithScaling::PrintSelf(ostream& os, vtkIndent indent)
{
...
int vtkGlyph3DWithScaling::RequestUpdateExtent(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **inputVector,
  vtkInformationVector *outputVector)
{
...
//----------------------------------------------------------------------------
vtkPolyData* vtkGlyph3DWithScaling::GetSource(int idx, vtkInformationVector *sourceInfo)
{
...
//----------------------------------------------------------------------------
int vtkGlyph3DWithScaling::FillInputPortInformation(int port, vtkInformation *info)
{
  if (port == 0)
    {
